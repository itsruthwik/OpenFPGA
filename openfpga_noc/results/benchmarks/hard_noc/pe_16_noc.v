

// `include "/mnt/vault1/rsunketa/OpenFPGA/openfpga-test-runs/000-push-button-flows/bench/koios_axi_wrap.v"
// `include "/mnt/vault1/rsunketa/OpenFPGA/openfpga_noc/results/benchmarks/koios_axi_wrap.v"

module pe_16_noc (
    input clk,
    input clk_noc,
    input reset,
    
    // AXI Stream input interface
    input [127:0] inp_data,
    input inp_valid,
    output inp_ready,
    input [3:0] inp_dest,
    input [1:0] inp_id,

    // AXI Stream output interface
    output [127:0] outp_data,
    output outp_valid,
    input outp_ready,
    output [3:0] outp_dest,
    output [1:0] outp_id,


// PE 0 signals
    output [127:0] pe0_out_data,
    output                     pe0_out_valid,
    output [3:0] pe0_out_dest,
    output [1:0]   pe0_out_id,
    output                     pe0_in_ready,
// PE 1 signals
    output [127:0] pe1_out_data,
    output                     pe1_out_valid,
    output [3:0] pe1_out_dest,
    output [1:0]   pe1_out_id,
    output                     pe1_in_ready,
// PE 2 signals
    output [127:0] pe2_out_data,
    output                     pe2_out_valid,
    output [3:0] pe2_out_dest,
    output [1:0]   pe2_out_id,
    output                     pe2_in_ready,
// PE 3 signals
    output [127:0] pe3_out_data,
    output                     pe3_out_valid,
    output [3:0] pe3_out_dest,
    output [1:0]   pe3_out_id,
    output                     pe3_in_ready,
// PE 4 signals
    output [127:0] pe4_out_data,
    output                     pe4_out_valid,
    output [3:0] pe4_out_dest,
    output [1:0]   pe4_out_id,
    output                     pe4_in_ready,
// PE 5 signals
    output [127:0] pe5_out_data,
    output                     pe5_out_valid,
    output [3:0] pe5_out_dest,
    output [1:0]   pe5_out_id,
    output                     pe5_in_ready,
// PE 6 signals
    output [127:0] pe6_out_data,
    output                     pe6_out_valid,
    output [3:0] pe6_out_dest,
    output [1:0]   pe6_out_id,
    output                     pe6_in_ready,
// PE 7 signals
    output [127:0] pe7_out_data,
    output                     pe7_out_valid,
    output [3:0] pe7_out_dest,
    output [1:0]   pe7_out_id,
    output                     pe7_in_ready,
// PE 8 signals
    output [127:0] pe8_out_data,
    output                     pe8_out_valid,
    output [3:0] pe8_out_dest,
    output [1:0]   pe8_out_id,
    output                     pe8_in_ready,
// PE 9 signals
    output [127:0] pe9_out_data,
    output                     pe9_out_valid,
    output [3:0] pe9_out_dest,
    output [1:0]   pe9_out_id,
    output                     pe9_in_ready,
// PE 10 signals
    output [127:0] pe10_out_data,
    output                     pe10_out_valid,
    output [3:0] pe10_out_dest,
    output [1:0]   pe10_out_id,
    output                     pe10_in_ready,
// PE 11 signals
    output [127:0] pe11_out_data,
    output                     pe11_out_valid,
    output [3:0] pe11_out_dest,
    output [1:0]   pe11_out_id,
    output                     pe11_in_ready,
// PE 12 signals
    output [127:0] pe12_out_data,
    output                     pe12_out_valid,
    output [3:0] pe12_out_dest,
    output [1:0]   pe12_out_id,
    output                     pe12_in_ready,
// PE 13 signals
    output [127:0] pe13_out_data,
    output                     pe13_out_valid,
    output [3:0] pe13_out_dest,
    output [1:0]   pe13_out_id,
    output                     pe13_in_ready,
// PE 14 signals
    output [127:0] pe14_out_data,
    output                     pe14_out_valid,
    output [3:0] pe14_out_dest,
    output [1:0]   pe14_out_id,
    output                     pe14_in_ready,
// PE 15 signals
    output [127:0] pe15_out_data,
    output                     pe15_out_valid,
    output [3:0] pe15_out_dest,
    output [1:0]   pe15_out_id,
    output                     pe15_in_ready,

    // Test signals for router 
    input  [16 :0] in_tlast,
    output [16 :0] out_tlast,
    // Test signals for all PEs
    input [63:0] test_input,
    output [63:0] test_output
);

    // Main router instantiation
    (* keep = "true" *) router_wrap rtr_main(
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),

        .axis_in_tdata(inp_data),
        .axis_in_tvalid(inp_valid),
        .axis_in_tready(inp_ready),
        .axis_in_tdest(inp_dest),
        .axis_in_tid(inp_id),
        .axis_in_tlast(in_tlast[0]),

        .axis_out_tdata(outp_data),
        .axis_out_tvalid(outp_valid),
        .axis_out_tready(outp_ready),
        .axis_out_tdest(outp_dest),
        .axis_out_tid(outp_id),
        .axis_out_tlast(out_tlast[0]),

        .router_address(4'b1111)
    );

// PE 0 signals
    // wire [127:0] pe0_out_data;
    // wire                     pe0_out_valid;
    wire                     pe0_out_ready;
    // wire [3:0] pe0_out_dest;
    // wire [1:0]   pe0_out_id;
    
    wire [127:0] pe0_in_data;
    wire                     pe0_in_valid;
    // wire                     pe0_in_ready;
    wire [3:0] pe0_in_dest;
    wire [1:0]   pe0_in_id;

// Router 0 signals
    wire [127:0] rtr0_in_data;
    wire                     rtr0_in_valid;
    wire                     rtr0_in_ready;
    wire [3:0] rtr0_in_dest;
    wire [1:0]   rtr0_in_id;

    wire [127:0] rtr0_out_data;
    wire                     rtr0_out_valid;
    wire                     rtr0_out_ready;
    wire [3:0] rtr0_out_dest;
    wire [1:0]   rtr0_out_id;

// PE 1 signals
    // wire [127:0] pe1_out_data;
    // wire                     pe1_out_valid;
    wire                     pe1_out_ready;
    // wire [3:0] pe1_out_dest;
    // wire [1:0]   pe1_out_id;
    
    wire [127:0] pe1_in_data;
    wire                     pe1_in_valid;
    // wire                     pe1_in_ready;
    wire [3:0] pe1_in_dest;
    wire [1:0]   pe1_in_id;

// Router 1 signals
    wire [127:0] rtr1_in_data;
    wire                     rtr1_in_valid;
    wire                     rtr1_in_ready;
    wire [3:0] rtr1_in_dest;
    wire [1:0]   rtr1_in_id;

    wire [127:0] rtr1_out_data;
    wire                     rtr1_out_valid;
    wire                     rtr1_out_ready;
    wire [3:0] rtr1_out_dest;
    wire [1:0]   rtr1_out_id;

// PE 2 signals
    // wire [127:0] pe2_out_data;
    // wire                     pe2_out_valid;
    wire                     pe2_out_ready;
    // wire [3:0] pe2_out_dest;
    // wire [1:0]   pe2_out_id;
    
    wire [127:0] pe2_in_data;
    wire                     pe2_in_valid;
    // wire                     pe2_in_ready;
    wire [3:0] pe2_in_dest;
    wire [1:0]   pe2_in_id;

// Router 2 signals
    wire [127:0] rtr2_in_data;
    wire                     rtr2_in_valid;
    wire                     rtr2_in_ready;
    wire [3:0] rtr2_in_dest;
    wire [1:0]   rtr2_in_id;

    wire [127:0] rtr2_out_data;
    wire                     rtr2_out_valid;
    wire                     rtr2_out_ready;
    wire [3:0] rtr2_out_dest;
    wire [1:0]   rtr2_out_id;

// PE 3 signals
    // wire [127:0] pe3_out_data;
    // wire                     pe3_out_valid;
    wire                     pe3_out_ready;
    // wire [3:0] pe3_out_dest;
    // wire [1:0]   pe3_out_id;
    
    wire [127:0] pe3_in_data;
    wire                     pe3_in_valid;
    // wire                     pe3_in_ready;
    wire [3:0] pe3_in_dest;
    wire [1:0]   pe3_in_id;

// Router 3 signals
    wire [127:0] rtr3_in_data;
    wire                     rtr3_in_valid;
    wire                     rtr3_in_ready;
    wire [3:0] rtr3_in_dest;
    wire [1:0]   rtr3_in_id;

    wire [127:0] rtr3_out_data;
    wire                     rtr3_out_valid;
    wire                     rtr3_out_ready;
    wire [3:0] rtr3_out_dest;
    wire [1:0]   rtr3_out_id;

// PE 4 signals
    // wire [127:0] pe4_out_data;
    // wire                     pe4_out_valid;
    wire                     pe4_out_ready;
    // wire [3:0] pe4_out_dest;
    // wire [1:0]   pe4_out_id;
    
    wire [127:0] pe4_in_data;
    wire                     pe4_in_valid;
    // wire                     pe4_in_ready;
    wire [3:0] pe4_in_dest;
    wire [1:0]   pe4_in_id;

// Router 4 signals
    wire [127:0] rtr4_in_data;
    wire                     rtr4_in_valid;
    wire                     rtr4_in_ready;
    wire [3:0] rtr4_in_dest;
    wire [1:0]   rtr4_in_id;

    wire [127:0] rtr4_out_data;
    wire                     rtr4_out_valid;
    wire                     rtr4_out_ready;
    wire [3:0] rtr4_out_dest;
    wire [1:0]   rtr4_out_id;

// PE 5 signals
    // wire [127:0] pe5_out_data;
    // wire                     pe5_out_valid;
    wire                     pe5_out_ready;
    // wire [3:0] pe5_out_dest;
    // wire [1:0]   pe5_out_id;
    
    wire [127:0] pe5_in_data;
    wire                     pe5_in_valid;
    // wire                     pe5_in_ready;
    wire [3:0] pe5_in_dest;
    wire [1:0]   pe5_in_id;

// Router 5 signals
    wire [127:0] rtr5_in_data;
    wire                     rtr5_in_valid;
    wire                     rtr5_in_ready;
    wire [3:0] rtr5_in_dest;
    wire [1:0]   rtr5_in_id;

    wire [127:0] rtr5_out_data;
    wire                     rtr5_out_valid;
    wire                     rtr5_out_ready;
    wire [3:0] rtr5_out_dest;
    wire [1:0]   rtr5_out_id;

// PE 6 signals
    // wire [127:0] pe6_out_data;
    // wire                     pe6_out_valid;
    wire                     pe6_out_ready;
    // wire [3:0] pe6_out_dest;
    // wire [1:0]   pe6_out_id;
    
    wire [127:0] pe6_in_data;
    wire                     pe6_in_valid;
    // wire                     pe6_in_ready;
    wire [3:0] pe6_in_dest;
    wire [1:0]   pe6_in_id;

// Router 6 signals
    wire [127:0] rtr6_in_data;
    wire                     rtr6_in_valid;
    wire                     rtr6_in_ready;
    wire [3:0] rtr6_in_dest;
    wire [1:0]   rtr6_in_id;

    wire [127:0] rtr6_out_data;
    wire                     rtr6_out_valid;
    wire                     rtr6_out_ready;
    wire [3:0] rtr6_out_dest;
    wire [1:0]   rtr6_out_id;

// PE 7 signals
    // wire [127:0] pe7_out_data;
    // wire                     pe7_out_valid;
    wire                     pe7_out_ready;
    // wire [3:0] pe7_out_dest;
    // wire [1:0]   pe7_out_id;
    
    wire [127:0] pe7_in_data;
    wire                     pe7_in_valid;
    // wire                     pe7_in_ready;
    wire [3:0] pe7_in_dest;
    wire [1:0]   pe7_in_id;

// Router 7 signals
    wire [127:0] rtr7_in_data;
    wire                     rtr7_in_valid;
    wire                     rtr7_in_ready;
    wire [3:0] rtr7_in_dest;
    wire [1:0]   rtr7_in_id;

    wire [127:0] rtr7_out_data;
    wire                     rtr7_out_valid;
    wire                     rtr7_out_ready;
    wire [3:0] rtr7_out_dest;
    wire [1:0]   rtr7_out_id;

// PE 8 signals
    // wire [127:0] pe8_out_data;
    // wire                     pe8_out_valid;
    wire                     pe8_out_ready;
    // wire [3:0] pe8_out_dest;
    // wire [1:0]   pe8_out_id;
    
    wire [127:0] pe8_in_data;
    wire                     pe8_in_valid;
    // wire                     pe8_in_ready;
    wire [3:0] pe8_in_dest;
    wire [1:0]   pe8_in_id;

// Router 8 signals
    wire [127:0] rtr8_in_data;
    wire                     rtr8_in_valid;
    wire                     rtr8_in_ready;
    wire [3:0] rtr8_in_dest;
    wire [1:0]   rtr8_in_id;

    wire [127:0] rtr8_out_data;
    wire                     rtr8_out_valid;
    wire                     rtr8_out_ready;
    wire [3:0] rtr8_out_dest;
    wire [1:0]   rtr8_out_id;

// PE 9 signals
    // wire [127:0] pe9_out_data;
    // wire                     pe9_out_valid;
    wire                     pe9_out_ready;
    // wire [3:0] pe9_out_dest;
    // wire [1:0]   pe9_out_id;
    
    wire [127:0] pe9_in_data;
    wire                     pe9_in_valid;
    // wire                     pe9_in_ready;
    wire [3:0] pe9_in_dest;
    wire [1:0]   pe9_in_id;

// Router 9 signals
    wire [127:0] rtr9_in_data;
    wire                     rtr9_in_valid;
    wire                     rtr9_in_ready;
    wire [3:0] rtr9_in_dest;
    wire [1:0]   rtr9_in_id;

    wire [127:0] rtr9_out_data;
    wire                     rtr9_out_valid;
    wire                     rtr9_out_ready;
    wire [3:0] rtr9_out_dest;
    wire [1:0]   rtr9_out_id;

// PE 10 signals
    // wire [127:0] pe10_out_data;
    // wire                     pe10_out_valid;
    wire                     pe10_out_ready;
    // wire [3:0] pe10_out_dest;
    // wire [1:0]   pe10_out_id;
    
    wire [127:0] pe10_in_data;
    wire                     pe10_in_valid;
    // wire                     pe10_in_ready;
    wire [3:0] pe10_in_dest;
    wire [1:0]   pe10_in_id;

// Router 10 signals
    wire [127:0] rtr10_in_data;
    wire                     rtr10_in_valid;
    wire                     rtr10_in_ready;
    wire [3:0] rtr10_in_dest;
    wire [1:0]   rtr10_in_id;

    wire [127:0] rtr10_out_data;
    wire                     rtr10_out_valid;
    wire                     rtr10_out_ready;
    wire [3:0] rtr10_out_dest;
    wire [1:0]   rtr10_out_id;

// PE 11 signals
    // wire [127:0] pe11_out_data;
    // wire                     pe11_out_valid;
    wire                     pe11_out_ready;
    // wire [3:0] pe11_out_dest;
    // wire [1:0]   pe11_out_id;
    
    wire [127:0] pe11_in_data;
    wire                     pe11_in_valid;
    // wire                     pe11_in_ready;
    wire [3:0] pe11_in_dest;
    wire [1:0]   pe11_in_id;

// Router 11 signals
    wire [127:0] rtr11_in_data;
    wire                     rtr11_in_valid;
    wire                     rtr11_in_ready;
    wire [3:0] rtr11_in_dest;
    wire [1:0]   rtr11_in_id;

    wire [127:0] rtr11_out_data;
    wire                     rtr11_out_valid;
    wire                     rtr11_out_ready;
    wire [3:0] rtr11_out_dest;
    wire [1:0]   rtr11_out_id;

// PE 12 signals
    // wire [127:0] pe12_out_data;
    // wire                     pe12_out_valid;
    wire                     pe12_out_ready;
    // wire [3:0] pe12_out_dest;
    // wire [1:0]   pe12_out_id;
    
    wire [127:0] pe12_in_data;
    wire                     pe12_in_valid;
    // wire                     pe12_in_ready;
    wire [3:0] pe12_in_dest;
    wire [1:0]   pe12_in_id;

// Router 12 signals
    wire [127:0] rtr12_in_data;
    wire                     rtr12_in_valid;
    wire                     rtr12_in_ready;
    wire [3:0] rtr12_in_dest;
    wire [1:0]   rtr12_in_id;

    wire [127:0] rtr12_out_data;
    wire                     rtr12_out_valid;
    wire                     rtr12_out_ready;
    wire [3:0] rtr12_out_dest;
    wire [1:0]   rtr12_out_id;

// PE 13 signals
    // wire [127:0] pe13_out_data;
    // wire                     pe13_out_valid;
    wire                     pe13_out_ready;
    // wire [3:0] pe13_out_dest;
    // wire [1:0]   pe13_out_id;
    
    wire [127:0] pe13_in_data;
    wire                     pe13_in_valid;
    // wire                     pe13_in_ready;
    wire [3:0] pe13_in_dest;
    wire [1:0]   pe13_in_id;

// Router 13 signals
    wire [127:0] rtr13_in_data;
    wire                     rtr13_in_valid;
    wire                     rtr13_in_ready;
    wire [3:0] rtr13_in_dest;
    wire [1:0]   rtr13_in_id;

    wire [127:0] rtr13_out_data;
    wire                     rtr13_out_valid;
    wire                     rtr13_out_ready;
    wire [3:0] rtr13_out_dest;
    wire [1:0]   rtr13_out_id;

// PE 14 signals
    // wire [127:0] pe14_out_data;
    // wire                     pe14_out_valid;
    wire                     pe14_out_ready;
    // wire [3:0] pe14_out_dest;
    // wire [1:0]   pe14_out_id;
    
    wire [127:0] pe14_in_data;
    wire                     pe14_in_valid;
    // wire                     pe14_in_ready;
    wire [3:0] pe14_in_dest;
    wire [1:0]   pe14_in_id;

// Router 14 signals
    wire [127:0] rtr14_in_data;
    wire                     rtr14_in_valid;
    wire                     rtr14_in_ready;
    wire [3:0] rtr14_in_dest;
    wire [1:0]   rtr14_in_id;

    wire [127:0] rtr14_out_data;
    wire                     rtr14_out_valid;
    wire                     rtr14_out_ready;
    wire [3:0] rtr14_out_dest;
    wire [1:0]   rtr14_out_id;

// PE 15 signals
    // wire [127:0] pe15_out_data;
    // wire                     pe15_out_valid;
    wire                     pe15_out_ready;
    // wire [3:0] pe15_out_dest;
    // wire [1:0]   pe15_out_id;
    
    wire [127:0] pe15_in_data;
    wire                     pe15_in_valid;
    // wire                     pe15_in_ready;
    wire [3:0] pe15_in_dest;
    wire [1:0]   pe15_in_id;

// Router 15 signals
    wire [127:0] rtr15_in_data;
    wire                     rtr15_in_valid;
    wire                     rtr15_in_ready;
    wire [3:0] rtr15_in_dest;
    wire [1:0]   rtr15_in_id;

    wire [127:0] rtr15_out_data;
    wire                     rtr15_out_valid;
    wire                     rtr15_out_ready;
    wire [3:0] rtr15_out_dest;
    wire [1:0]   rtr15_out_id;


// PE 0 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_0 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe0_in_data),
        .axis_in_valid(pe0_in_valid),
        .axis_in_ready(pe0_in_ready),
        // .inp_dest(pe0_in_dest),
        .axis_in_id(pe0_in_id),
        // .axis_in_last(pe0_in_last),

        .axis_out_data(pe0_out_data),
        .axis_out_valid(pe0_out_valid),
        .axis_out_ready(pe0_out_ready),
        // .axis_out_dest(pe0_out_dest),
        .axis_out_id(pe0_out_id),
        .axis_out_id(pe0_out_id),
        // .axis_out_last(pe0_out_last)

        // .test_input(test_input[3:0]),
        // .test_output(test_output[3:0])
    );
    
// Router 0 instantiation
    (* keep = "true" *) router_wrap rtr_inst_0 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr0_in_data),
        .axis_in_tvalid(rtr0_in_valid),
        .axis_in_tready(rtr0_in_ready),
        .axis_in_tdest(rtr0_in_dest),
        .axis_in_tid(rtr0_in_id),
        .axis_in_tlast(in_tlast[1]),
        
        .axis_out_tdata(rtr0_out_data),
        .axis_out_tvalid(rtr0_out_valid),
        .axis_out_tready(rtr0_out_ready),
        .axis_out_tdest(rtr0_out_dest),
        .axis_out_tid(rtr0_out_id),
        .axis_out_tlast(out_tlast[1]),
        
        .router_address(4'd0)
    );

// PE 1 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_1 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe1_in_data),
        .axis_in_valid(pe1_in_valid),
        .axis_in_ready(pe1_in_ready),
        // .inp_dest(pe1_in_dest),
        .axis_in_id(pe1_in_id),
        // .axis_in_last(pe1_in_last),

        .axis_out_data(pe1_out_data),
        .axis_out_valid(pe1_out_valid),
        .axis_out_ready(pe1_out_ready),
        // .axis_out_dest(pe1_out_dest),
        .axis_out_id(pe1_out_id),
        .axis_out_id(pe1_out_id),
        // .axis_out_last(pe1_out_last)

        // .test_input(test_input[7:4]),
        // .test_output(test_output[7:4])
    );
    
// Router 1 instantiation
    (* keep = "true" *) router_wrap rtr_inst_1 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr1_in_data),
        .axis_in_tvalid(rtr1_in_valid),
        .axis_in_tready(rtr1_in_ready),
        .axis_in_tdest(rtr1_in_dest),
        .axis_in_tid(rtr1_in_id),
        .axis_in_tlast(in_tlast[2]),
        
        .axis_out_tdata(rtr1_out_data),
        .axis_out_tvalid(rtr1_out_valid),
        .axis_out_tready(rtr1_out_ready),
        .axis_out_tdest(rtr1_out_dest),
        .axis_out_tid(rtr1_out_id),
        .axis_out_tlast(out_tlast[2]),
        
        .router_address(4'd1)
    );

// PE 2 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_2 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe2_in_data),
        .axis_in_valid(pe2_in_valid),
        .axis_in_ready(pe2_in_ready),
        // .inp_dest(pe2_in_dest),
        .axis_in_id(pe2_in_id),
        // .axis_in_last(pe2_in_last),

        .axis_out_data(pe2_out_data),
        .axis_out_valid(pe2_out_valid),
        .axis_out_ready(pe2_out_ready),
        // .axis_out_dest(pe2_out_dest),
        .axis_out_id(pe2_out_id),
        .axis_out_id(pe2_out_id),
        // .axis_out_last(pe2_out_last)

        // .test_input(test_input[11:8]),
        // .test_output(test_output[11:8])
    );
    
// Router 2 instantiation
    (* keep = "true" *) router_wrap rtr_inst_2 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr2_in_data),
        .axis_in_tvalid(rtr2_in_valid),
        .axis_in_tready(rtr2_in_ready),
        .axis_in_tdest(rtr2_in_dest),
        .axis_in_tid(rtr2_in_id),
        .axis_in_tlast(in_tlast[3]),
        
        .axis_out_tdata(rtr2_out_data),
        .axis_out_tvalid(rtr2_out_valid),
        .axis_out_tready(rtr2_out_ready),
        .axis_out_tdest(rtr2_out_dest),
        .axis_out_tid(rtr2_out_id),
        .axis_out_tlast(out_tlast[3]),
        
        .router_address(4'd2)
    );

// PE 3 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_3 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe3_in_data),
        .axis_in_valid(pe3_in_valid),
        .axis_in_ready(pe3_in_ready),
        // .inp_dest(pe3_in_dest),
        .axis_in_id(pe3_in_id),
        // .axis_in_last(pe3_in_last),

        .axis_out_data(pe3_out_data),
        .axis_out_valid(pe3_out_valid),
        .axis_out_ready(pe3_out_ready),
        // .axis_out_dest(pe3_out_dest),
        .axis_out_id(pe3_out_id),
        .axis_out_id(pe3_out_id),
        // .axis_out_last(pe3_out_last)

        // .test_input(test_input[15:12]),
        // .test_output(test_output[15:12])
    );
    
// Router 3 instantiation
    (* keep = "true" *) router_wrap rtr_inst_3 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr3_in_data),
        .axis_in_tvalid(rtr3_in_valid),
        .axis_in_tready(rtr3_in_ready),
        .axis_in_tdest(rtr3_in_dest),
        .axis_in_tid(rtr3_in_id),
        .axis_in_tlast(in_tlast[4]),
        
        .axis_out_tdata(rtr3_out_data),
        .axis_out_tvalid(rtr3_out_valid),
        .axis_out_tready(rtr3_out_ready),
        .axis_out_tdest(rtr3_out_dest),
        .axis_out_tid(rtr3_out_id),
        .axis_out_tlast(out_tlast[4]),
        
        .router_address(4'd3)
    );

// PE 4 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_4 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe4_in_data),
        .axis_in_valid(pe4_in_valid),
        .axis_in_ready(pe4_in_ready),
        // .inp_dest(pe4_in_dest),
        .axis_in_id(pe4_in_id),
        // .axis_in_last(pe4_in_last),

        .axis_out_data(pe4_out_data),
        .axis_out_valid(pe4_out_valid),
        .axis_out_ready(pe4_out_ready),
        // .axis_out_dest(pe4_out_dest),
        .axis_out_id(pe4_out_id),
        .axis_out_id(pe4_out_id),
        // .axis_out_last(pe4_out_last)

        // .test_input(test_input[19:16]),
        // .test_output(test_output[19:16])
    );
    
// Router 4 instantiation
    (* keep = "true" *) router_wrap rtr_inst_4 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr4_in_data),
        .axis_in_tvalid(rtr4_in_valid),
        .axis_in_tready(rtr4_in_ready),
        .axis_in_tdest(rtr4_in_dest),
        .axis_in_tid(rtr4_in_id),
        .axis_in_tlast(in_tlast[5]),
        
        .axis_out_tdata(rtr4_out_data),
        .axis_out_tvalid(rtr4_out_valid),
        .axis_out_tready(rtr4_out_ready),
        .axis_out_tdest(rtr4_out_dest),
        .axis_out_tid(rtr4_out_id),
        .axis_out_tlast(out_tlast[5]),
        
        .router_address(4'd4)
    );

// PE 5 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_5 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe5_in_data),
        .axis_in_valid(pe5_in_valid),
        .axis_in_ready(pe5_in_ready),
        // .inp_dest(pe5_in_dest),
        .axis_in_id(pe5_in_id),
        // .axis_in_last(pe5_in_last),

        .axis_out_data(pe5_out_data),
        .axis_out_valid(pe5_out_valid),
        .axis_out_ready(pe5_out_ready),
        // .axis_out_dest(pe5_out_dest),
        .axis_out_id(pe5_out_id),
        .axis_out_id(pe5_out_id),
        // .axis_out_last(pe5_out_last)

        // .test_input(test_input[23:20]),
        // .test_output(test_output[23:20])
    );
    
// Router 5 instantiation
    (* keep = "true" *) router_wrap rtr_inst_5 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr5_in_data),
        .axis_in_tvalid(rtr5_in_valid),
        .axis_in_tready(rtr5_in_ready),
        .axis_in_tdest(rtr5_in_dest),
        .axis_in_tid(rtr5_in_id),
        .axis_in_tlast(in_tlast[6]),
        
        .axis_out_tdata(rtr5_out_data),
        .axis_out_tvalid(rtr5_out_valid),
        .axis_out_tready(rtr5_out_ready),
        .axis_out_tdest(rtr5_out_dest),
        .axis_out_tid(rtr5_out_id),
        .axis_out_tlast(out_tlast[6]),
        
        .router_address(4'd5)
    );

// PE 6 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_6 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe6_in_data),
        .axis_in_valid(pe6_in_valid),
        .axis_in_ready(pe6_in_ready),
        // .inp_dest(pe6_in_dest),
        .axis_in_id(pe6_in_id),
        // .axis_in_last(pe6_in_last),

        .axis_out_data(pe6_out_data),
        .axis_out_valid(pe6_out_valid),
        .axis_out_ready(pe6_out_ready),
        // .axis_out_dest(pe6_out_dest),
        .axis_out_id(pe6_out_id),
        .axis_out_id(pe6_out_id),
        // .axis_out_last(pe6_out_last)

        // .test_input(test_input[27:24]),
        // .test_output(test_output[27:24])
    );
    
// Router 6 instantiation
    (* keep = "true" *) router_wrap rtr_inst_6 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr6_in_data),
        .axis_in_tvalid(rtr6_in_valid),
        .axis_in_tready(rtr6_in_ready),
        .axis_in_tdest(rtr6_in_dest),
        .axis_in_tid(rtr6_in_id),
        .axis_in_tlast(in_tlast[7]),
        
        .axis_out_tdata(rtr6_out_data),
        .axis_out_tvalid(rtr6_out_valid),
        .axis_out_tready(rtr6_out_ready),
        .axis_out_tdest(rtr6_out_dest),
        .axis_out_tid(rtr6_out_id),
        .axis_out_tlast(out_tlast[7]),
        
        .router_address(4'd6)
    );

// PE 7 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_7 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe7_in_data),
        .axis_in_valid(pe7_in_valid),
        .axis_in_ready(pe7_in_ready),
        // .inp_dest(pe7_in_dest),
        .axis_in_id(pe7_in_id),
        // .axis_in_last(pe7_in_last),

        .axis_out_data(pe7_out_data),
        .axis_out_valid(pe7_out_valid),
        .axis_out_ready(pe7_out_ready),
        // .axis_out_dest(pe7_out_dest),
        .axis_out_id(pe7_out_id),
        .axis_out_id(pe7_out_id),
        // .axis_out_last(pe7_out_last)

        // .test_input(test_input[31:28]),
        // .test_output(test_output[31:28])
    );
    
// Router 7 instantiation
    (* keep = "true" *) router_wrap rtr_inst_7 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr7_in_data),
        .axis_in_tvalid(rtr7_in_valid),
        .axis_in_tready(rtr7_in_ready),
        .axis_in_tdest(rtr7_in_dest),
        .axis_in_tid(rtr7_in_id),
        .axis_in_tlast(in_tlast[8]),
        
        .axis_out_tdata(rtr7_out_data),
        .axis_out_tvalid(rtr7_out_valid),
        .axis_out_tready(rtr7_out_ready),
        .axis_out_tdest(rtr7_out_dest),
        .axis_out_tid(rtr7_out_id),
        .axis_out_tlast(out_tlast[8]),
        
        .router_address(4'd7)
    );

// PE 8 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_8 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe8_in_data),
        .axis_in_valid(pe8_in_valid),
        .axis_in_ready(pe8_in_ready),
        // .inp_dest(pe8_in_dest),
        .axis_in_id(pe8_in_id),
        // .axis_in_last(pe8_in_last),

        .axis_out_data(pe8_out_data),
        .axis_out_valid(pe8_out_valid),
        .axis_out_ready(pe8_out_ready),
        // .axis_out_dest(pe8_out_dest),
        .axis_out_id(pe8_out_id),
        .axis_out_id(pe8_out_id),
        // .axis_out_last(pe8_out_last)

        // .test_input(test_input[35:32]),
        // .test_output(test_output[35:32])
    );
    
// Router 8 instantiation
    (* keep = "true" *) router_wrap rtr_inst_8 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr8_in_data),
        .axis_in_tvalid(rtr8_in_valid),
        .axis_in_tready(rtr8_in_ready),
        .axis_in_tdest(rtr8_in_dest),
        .axis_in_tid(rtr8_in_id),
        .axis_in_tlast(in_tlast[9]),
        
        .axis_out_tdata(rtr8_out_data),
        .axis_out_tvalid(rtr8_out_valid),
        .axis_out_tready(rtr8_out_ready),
        .axis_out_tdest(rtr8_out_dest),
        .axis_out_tid(rtr8_out_id),
        .axis_out_tlast(out_tlast[9]),
        
        .router_address(4'd8)
    );

// PE 9 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_9 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe9_in_data),
        .axis_in_valid(pe9_in_valid),
        .axis_in_ready(pe9_in_ready),
        // .inp_dest(pe9_in_dest),
        .axis_in_id(pe9_in_id),
        // .axis_in_last(pe9_in_last),

        .axis_out_data(pe9_out_data),
        .axis_out_valid(pe9_out_valid),
        .axis_out_ready(pe9_out_ready),
        // .axis_out_dest(pe9_out_dest),
        .axis_out_id(pe9_out_id),
        .axis_out_id(pe9_out_id),
        // .axis_out_last(pe9_out_last)

        // .test_input(test_input[39:36]),
        // .test_output(test_output[39:36])
    );
    
// Router 9 instantiation
    (* keep = "true" *) router_wrap rtr_inst_9 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr9_in_data),
        .axis_in_tvalid(rtr9_in_valid),
        .axis_in_tready(rtr9_in_ready),
        .axis_in_tdest(rtr9_in_dest),
        .axis_in_tid(rtr9_in_id),
        .axis_in_tlast(in_tlast[10]),
        
        .axis_out_tdata(rtr9_out_data),
        .axis_out_tvalid(rtr9_out_valid),
        .axis_out_tready(rtr9_out_ready),
        .axis_out_tdest(rtr9_out_dest),
        .axis_out_tid(rtr9_out_id),
        .axis_out_tlast(out_tlast[10]),
        
        .router_address(4'd9)
    );

// PE 10 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_10 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe10_in_data),
        .axis_in_valid(pe10_in_valid),
        .axis_in_ready(pe10_in_ready),
        // .inp_dest(pe10_in_dest),
        .axis_in_id(pe10_in_id),
        // .axis_in_last(pe10_in_last),

        .axis_out_data(pe10_out_data),
        .axis_out_valid(pe10_out_valid),
        .axis_out_ready(pe10_out_ready),
        // .axis_out_dest(pe10_out_dest),
        .axis_out_id(pe10_out_id),
        .axis_out_id(pe10_out_id),
        // .axis_out_last(pe10_out_last)

        // .test_input(test_input[43:40]),
        // .test_output(test_output[43:40])
    );
    
// Router 10 instantiation
    (* keep = "true" *) router_wrap rtr_inst_10 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr10_in_data),
        .axis_in_tvalid(rtr10_in_valid),
        .axis_in_tready(rtr10_in_ready),
        .axis_in_tdest(rtr10_in_dest),
        .axis_in_tid(rtr10_in_id),
        .axis_in_tlast(in_tlast[11]),
        
        .axis_out_tdata(rtr10_out_data),
        .axis_out_tvalid(rtr10_out_valid),
        .axis_out_tready(rtr10_out_ready),
        .axis_out_tdest(rtr10_out_dest),
        .axis_out_tid(rtr10_out_id),
        .axis_out_tlast(out_tlast[11]),
        
        .router_address(4'd10)
    );

// PE 11 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_11 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe11_in_data),
        .axis_in_valid(pe11_in_valid),
        .axis_in_ready(pe11_in_ready),
        // .inp_dest(pe11_in_dest),
        .axis_in_id(pe11_in_id),
        // .axis_in_last(pe11_in_last),

        .axis_out_data(pe11_out_data),
        .axis_out_valid(pe11_out_valid),
        .axis_out_ready(pe11_out_ready),
        // .axis_out_dest(pe11_out_dest),
        .axis_out_id(pe11_out_id),
        .axis_out_id(pe11_out_id),
        // .axis_out_last(pe11_out_last)

        // .test_input(test_input[47:44]),
        // .test_output(test_output[47:44])
    );
    
// Router 11 instantiation
    (* keep = "true" *) router_wrap rtr_inst_11 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr11_in_data),
        .axis_in_tvalid(rtr11_in_valid),
        .axis_in_tready(rtr11_in_ready),
        .axis_in_tdest(rtr11_in_dest),
        .axis_in_tid(rtr11_in_id),
        .axis_in_tlast(in_tlast[12]),
        
        .axis_out_tdata(rtr11_out_data),
        .axis_out_tvalid(rtr11_out_valid),
        .axis_out_tready(rtr11_out_ready),
        .axis_out_tdest(rtr11_out_dest),
        .axis_out_tid(rtr11_out_id),
        .axis_out_tlast(out_tlast[12]),
        
        .router_address(4'd11)
    );

// PE 12 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_12 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe12_in_data),
        .axis_in_valid(pe12_in_valid),
        .axis_in_ready(pe12_in_ready),
        // .inp_dest(pe12_in_dest),
        .axis_in_id(pe12_in_id),
        // .axis_in_last(pe12_in_last),

        .axis_out_data(pe12_out_data),
        .axis_out_valid(pe12_out_valid),
        .axis_out_ready(pe12_out_ready),
        // .axis_out_dest(pe12_out_dest),
        .axis_out_id(pe12_out_id),
        .axis_out_id(pe12_out_id),
        // .axis_out_last(pe12_out_last)

        // .test_input(test_input[51:48]),
        // .test_output(test_output[51:48])
    );
    
// Router 12 instantiation
    (* keep = "true" *) router_wrap rtr_inst_12 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr12_in_data),
        .axis_in_tvalid(rtr12_in_valid),
        .axis_in_tready(rtr12_in_ready),
        .axis_in_tdest(rtr12_in_dest),
        .axis_in_tid(rtr12_in_id),
        .axis_in_tlast(in_tlast[13]),
        
        .axis_out_tdata(rtr12_out_data),
        .axis_out_tvalid(rtr12_out_valid),
        .axis_out_tready(rtr12_out_ready),
        .axis_out_tdest(rtr12_out_dest),
        .axis_out_tid(rtr12_out_id),
        .axis_out_tlast(out_tlast[13]),
        
        .router_address(4'd12)
    );

// PE 13 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_13 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe13_in_data),
        .axis_in_valid(pe13_in_valid),
        .axis_in_ready(pe13_in_ready),
        // .inp_dest(pe13_in_dest),
        .axis_in_id(pe13_in_id),
        // .axis_in_last(pe13_in_last),

        .axis_out_data(pe13_out_data),
        .axis_out_valid(pe13_out_valid),
        .axis_out_ready(pe13_out_ready),
        // .axis_out_dest(pe13_out_dest),
        .axis_out_id(pe13_out_id),
        .axis_out_id(pe13_out_id),
        // .axis_out_last(pe13_out_last)

        // .test_input(test_input[55:52]),
        // .test_output(test_output[55:52])
    );
    
// Router 13 instantiation
    (* keep = "true" *) router_wrap rtr_inst_13 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr13_in_data),
        .axis_in_tvalid(rtr13_in_valid),
        .axis_in_tready(rtr13_in_ready),
        .axis_in_tdest(rtr13_in_dest),
        .axis_in_tid(rtr13_in_id),
        .axis_in_tlast(in_tlast[14]),
        
        .axis_out_tdata(rtr13_out_data),
        .axis_out_tvalid(rtr13_out_valid),
        .axis_out_tready(rtr13_out_ready),
        .axis_out_tdest(rtr13_out_dest),
        .axis_out_tid(rtr13_out_id),
        .axis_out_tlast(out_tlast[14]),
        
        .router_address(4'd13)
    );

// PE 14 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_14 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe14_in_data),
        .axis_in_valid(pe14_in_valid),
        .axis_in_ready(pe14_in_ready),
        // .inp_dest(pe14_in_dest),
        .axis_in_id(pe14_in_id),
        // .axis_in_last(pe14_in_last),

        .axis_out_data(pe14_out_data),
        .axis_out_valid(pe14_out_valid),
        .axis_out_ready(pe14_out_ready),
        // .axis_out_dest(pe14_out_dest),
        .axis_out_id(pe14_out_id),
        .axis_out_id(pe14_out_id),
        // .axis_out_last(pe14_out_last)

        // .test_input(test_input[59:56]),
        // .test_output(test_output[59:56])
    );
    
// Router 14 instantiation
    (* keep = "true" *) router_wrap rtr_inst_14 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr14_in_data),
        .axis_in_tvalid(rtr14_in_valid),
        .axis_in_tready(rtr14_in_ready),
        .axis_in_tdest(rtr14_in_dest),
        .axis_in_tid(rtr14_in_id),
        .axis_in_tlast(in_tlast[15]),
        
        .axis_out_tdata(rtr14_out_data),
        .axis_out_tvalid(rtr14_out_valid),
        .axis_out_tready(rtr14_out_ready),
        .axis_out_tdest(rtr14_out_dest),
        .axis_out_tid(rtr14_out_id),
        .axis_out_tlast(out_tlast[15]),
        
        .router_address(4'd14)
    );

// PE 15 instantiation
    (* keep = "true" *) koios_axi_wrap pe_inst_15 (
        .clk(clk),
        .reset(reset),
        
        .axis_in_data(pe15_in_data),
        .axis_in_valid(pe15_in_valid),
        .axis_in_ready(pe15_in_ready),
        // .inp_dest(pe15_in_dest),
        .axis_in_id(pe15_in_id),
        // .axis_in_last(pe15_in_last),

        .axis_out_data(pe15_out_data),
        .axis_out_valid(pe15_out_valid),
        .axis_out_ready(pe15_out_ready),
        // .axis_out_dest(pe15_out_dest),
        .axis_out_id(pe15_out_id),
        .axis_out_id(pe15_out_id),
        // .axis_out_last(pe15_out_last)

        // .test_input(test_input[63:60]),
        // .test_output(test_output[63:60])
    );
    
// Router 15 instantiation
    (* keep = "true" *) router_wrap rtr_inst_15 (
        .clk_usr(clk),
        .clk_noc(clk_noc),
        .rst_n(reset),
        
        .axis_in_tdata(rtr15_in_data),
        .axis_in_tvalid(rtr15_in_valid),
        .axis_in_tready(rtr15_in_ready),
        .axis_in_tdest(rtr15_in_dest),
        .axis_in_tid(rtr15_in_id),
        .axis_in_tlast(in_tlast[16]),
        
        .axis_out_tdata(rtr15_out_data),
        .axis_out_tvalid(rtr15_out_valid),
        .axis_out_tready(rtr15_out_ready),
        .axis_out_tdest(rtr15_out_dest),
        .axis_out_tid(rtr15_out_id),
        .axis_out_tlast(out_tlast[16]),
        
        .router_address(4'd15)
    );




// PE 0 to rtr 0 
    // Connect PE 0 output to Router 0 input
    assign rtr0_in_data = pe0_out_data;
    assign rtr0_in_valid = pe0_out_valid;
    assign rtr0_in_dest = pe0_out_dest;
    assign rtr0_in_id = pe0_out_id;
    assign pe0_out_ready = rtr0_in_ready; // ready is from the router

    // Connect Router 0 output to PE 0 input
    assign pe0_in_data = rtr0_out_data;
    assign pe0_in_valid = rtr0_out_valid;
    assign pe0_in_dest = rtr0_out_dest;
    assign pe0_in_id = rtr0_out_id;
    assign rtr0_out_ready = pe0_in_ready; // ready is from the PE 


// PE 1 to rtr 1 
    // Connect PE 1 output to Router 1 input
    assign rtr1_in_data = pe1_out_data;
    assign rtr1_in_valid = pe1_out_valid;
    assign rtr1_in_dest = pe1_out_dest;
    assign rtr1_in_id = pe1_out_id;
    assign pe1_out_ready = rtr1_in_ready; // ready is from the router

    // Connect Router 1 output to PE 1 input
    assign pe1_in_data = rtr1_out_data;
    assign pe1_in_valid = rtr1_out_valid;
    assign pe1_in_dest = rtr1_out_dest;
    assign pe1_in_id = rtr1_out_id;
    assign rtr1_out_ready = pe1_in_ready; // ready is from the PE 


// PE 2 to rtr 2 
    // Connect PE 2 output to Router 2 input
    assign rtr2_in_data = pe2_out_data;
    assign rtr2_in_valid = pe2_out_valid;
    assign rtr2_in_dest = pe2_out_dest;
    assign rtr2_in_id = pe2_out_id;
    assign pe2_out_ready = rtr2_in_ready; // ready is from the router

    // Connect Router 2 output to PE 2 input
    assign pe2_in_data = rtr2_out_data;
    assign pe2_in_valid = rtr2_out_valid;
    assign pe2_in_dest = rtr2_out_dest;
    assign pe2_in_id = rtr2_out_id;
    assign rtr2_out_ready = pe2_in_ready; // ready is from the PE 


// PE 3 to rtr 3 
    // Connect PE 3 output to Router 3 input
    assign rtr3_in_data = pe3_out_data;
    assign rtr3_in_valid = pe3_out_valid;
    assign rtr3_in_dest = pe3_out_dest;
    assign rtr3_in_id = pe3_out_id;
    assign pe3_out_ready = rtr3_in_ready; // ready is from the router

    // Connect Router 3 output to PE 3 input
    assign pe3_in_data = rtr3_out_data;
    assign pe3_in_valid = rtr3_out_valid;
    assign pe3_in_dest = rtr3_out_dest;
    assign pe3_in_id = rtr3_out_id;
    assign rtr3_out_ready = pe3_in_ready; // ready is from the PE 


// PE 4 to rtr 4 
    // Connect PE 4 output to Router 4 input
    assign rtr4_in_data = pe4_out_data;
    assign rtr4_in_valid = pe4_out_valid;
    assign rtr4_in_dest = pe4_out_dest;
    assign rtr4_in_id = pe4_out_id;
    assign pe4_out_ready = rtr4_in_ready; // ready is from the router

    // Connect Router 4 output to PE 4 input
    assign pe4_in_data = rtr4_out_data;
    assign pe4_in_valid = rtr4_out_valid;
    assign pe4_in_dest = rtr4_out_dest;
    assign pe4_in_id = rtr4_out_id;
    assign rtr4_out_ready = pe4_in_ready; // ready is from the PE 


// PE 5 to rtr 5 
    // Connect PE 5 output to Router 5 input
    assign rtr5_in_data = pe5_out_data;
    assign rtr5_in_valid = pe5_out_valid;
    assign rtr5_in_dest = pe5_out_dest;
    assign rtr5_in_id = pe5_out_id;
    assign pe5_out_ready = rtr5_in_ready; // ready is from the router

    // Connect Router 5 output to PE 5 input
    assign pe5_in_data = rtr5_out_data;
    assign pe5_in_valid = rtr5_out_valid;
    assign pe5_in_dest = rtr5_out_dest;
    assign pe5_in_id = rtr5_out_id;
    assign rtr5_out_ready = pe5_in_ready; // ready is from the PE 


// PE 6 to rtr 6 
    // Connect PE 6 output to Router 6 input
    assign rtr6_in_data = pe6_out_data;
    assign rtr6_in_valid = pe6_out_valid;
    assign rtr6_in_dest = pe6_out_dest;
    assign rtr6_in_id = pe6_out_id;
    assign pe6_out_ready = rtr6_in_ready; // ready is from the router

    // Connect Router 6 output to PE 6 input
    assign pe6_in_data = rtr6_out_data;
    assign pe6_in_valid = rtr6_out_valid;
    assign pe6_in_dest = rtr6_out_dest;
    assign pe6_in_id = rtr6_out_id;
    assign rtr6_out_ready = pe6_in_ready; // ready is from the PE 


// PE 7 to rtr 7 
    // Connect PE 7 output to Router 7 input
    assign rtr7_in_data = pe7_out_data;
    assign rtr7_in_valid = pe7_out_valid;
    assign rtr7_in_dest = pe7_out_dest;
    assign rtr7_in_id = pe7_out_id;
    assign pe7_out_ready = rtr7_in_ready; // ready is from the router

    // Connect Router 7 output to PE 7 input
    assign pe7_in_data = rtr7_out_data;
    assign pe7_in_valid = rtr7_out_valid;
    assign pe7_in_dest = rtr7_out_dest;
    assign pe7_in_id = rtr7_out_id;
    assign rtr7_out_ready = pe7_in_ready; // ready is from the PE 


// PE 8 to rtr 8 
    // Connect PE 8 output to Router 8 input
    assign rtr8_in_data = pe8_out_data;
    assign rtr8_in_valid = pe8_out_valid;
    assign rtr8_in_dest = pe8_out_dest;
    assign rtr8_in_id = pe8_out_id;
    assign pe8_out_ready = rtr8_in_ready; // ready is from the router

    // Connect Router 8 output to PE 8 input
    assign pe8_in_data = rtr8_out_data;
    assign pe8_in_valid = rtr8_out_valid;
    assign pe8_in_dest = rtr8_out_dest;
    assign pe8_in_id = rtr8_out_id;
    assign rtr8_out_ready = pe8_in_ready; // ready is from the PE 


// PE 9 to rtr 9 
    // Connect PE 9 output to Router 9 input
    assign rtr9_in_data = pe9_out_data;
    assign rtr9_in_valid = pe9_out_valid;
    assign rtr9_in_dest = pe9_out_dest;
    assign rtr9_in_id = pe9_out_id;
    assign pe9_out_ready = rtr9_in_ready; // ready is from the router

    // Connect Router 9 output to PE 9 input
    assign pe9_in_data = rtr9_out_data;
    assign pe9_in_valid = rtr9_out_valid;
    assign pe9_in_dest = rtr9_out_dest;
    assign pe9_in_id = rtr9_out_id;
    assign rtr9_out_ready = pe9_in_ready; // ready is from the PE 


// PE 10 to rtr 10 
    // Connect PE 10 output to Router 10 input
    assign rtr10_in_data = pe10_out_data;
    assign rtr10_in_valid = pe10_out_valid;
    assign rtr10_in_dest = pe10_out_dest;
    assign rtr10_in_id = pe10_out_id;
    assign pe10_out_ready = rtr10_in_ready; // ready is from the router

    // Connect Router 10 output to PE 10 input
    assign pe10_in_data = rtr10_out_data;
    assign pe10_in_valid = rtr10_out_valid;
    assign pe10_in_dest = rtr10_out_dest;
    assign pe10_in_id = rtr10_out_id;
    assign rtr10_out_ready = pe10_in_ready; // ready is from the PE 


// PE 11 to rtr 11 
    // Connect PE 11 output to Router 11 input
    assign rtr11_in_data = pe11_out_data;
    assign rtr11_in_valid = pe11_out_valid;
    assign rtr11_in_dest = pe11_out_dest;
    assign rtr11_in_id = pe11_out_id;
    assign pe11_out_ready = rtr11_in_ready; // ready is from the router

    // Connect Router 11 output to PE 11 input
    assign pe11_in_data = rtr11_out_data;
    assign pe11_in_valid = rtr11_out_valid;
    assign pe11_in_dest = rtr11_out_dest;
    assign pe11_in_id = rtr11_out_id;
    assign rtr11_out_ready = pe11_in_ready; // ready is from the PE 


// PE 12 to rtr 12 
    // Connect PE 12 output to Router 12 input
    assign rtr12_in_data = pe12_out_data;
    assign rtr12_in_valid = pe12_out_valid;
    assign rtr12_in_dest = pe12_out_dest;
    assign rtr12_in_id = pe12_out_id;
    assign pe12_out_ready = rtr12_in_ready; // ready is from the router

    // Connect Router 12 output to PE 12 input
    assign pe12_in_data = rtr12_out_data;
    assign pe12_in_valid = rtr12_out_valid;
    assign pe12_in_dest = rtr12_out_dest;
    assign pe12_in_id = rtr12_out_id;
    assign rtr12_out_ready = pe12_in_ready; // ready is from the PE 


// PE 13 to rtr 13 
    // Connect PE 13 output to Router 13 input
    assign rtr13_in_data = pe13_out_data;
    assign rtr13_in_valid = pe13_out_valid;
    assign rtr13_in_dest = pe13_out_dest;
    assign rtr13_in_id = pe13_out_id;
    assign pe13_out_ready = rtr13_in_ready; // ready is from the router

    // Connect Router 13 output to PE 13 input
    assign pe13_in_data = rtr13_out_data;
    assign pe13_in_valid = rtr13_out_valid;
    assign pe13_in_dest = rtr13_out_dest;
    assign pe13_in_id = rtr13_out_id;
    assign rtr13_out_ready = pe13_in_ready; // ready is from the PE 


// PE 14 to rtr 14 
    // Connect PE 14 output to Router 14 input
    assign rtr14_in_data = pe14_out_data;
    assign rtr14_in_valid = pe14_out_valid;
    assign rtr14_in_dest = pe14_out_dest;
    assign rtr14_in_id = pe14_out_id;
    assign pe14_out_ready = rtr14_in_ready; // ready is from the router

    // Connect Router 14 output to PE 14 input
    assign pe14_in_data = rtr14_out_data;
    assign pe14_in_valid = rtr14_out_valid;
    assign pe14_in_dest = rtr14_out_dest;
    assign pe14_in_id = rtr14_out_id;
    assign rtr14_out_ready = pe14_in_ready; // ready is from the PE 


// PE 15 to rtr 15 
    // Connect PE 15 output to Router 15 input
    assign rtr15_in_data = pe15_out_data;
    assign rtr15_in_valid = pe15_out_valid;
    assign rtr15_in_dest = pe15_out_dest;
    assign rtr15_in_id = pe15_out_id;
    assign pe15_out_ready = rtr15_in_ready; // ready is from the router

    // Connect Router 15 output to PE 15 input
    assign pe15_in_data = rtr15_out_data;
    assign pe15_in_valid = rtr15_out_valid;
    assign pe15_in_dest = rtr15_out_dest;
    assign pe15_in_id = rtr15_out_id;
    assign rtr15_out_ready = pe15_in_ready; // ready is from the PE 

endmodule