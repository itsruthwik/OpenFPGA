...
def update_xml(port_dict, top, has_clock, clock_port, arch_dir):
    ...
    tree = ET.parse(f'{arch_dir}/openfpga_arch.xml', parser)
    # tree = ET.parse(f'{arch_dir}/openfpga_arch.xml')
    root = tree.getroot()
    ...
    tree.write(f'{arch_dir}/openfpga_arch.xml', pretty_print=True, xml_declaration=True, encoding='UTF-8')
    print(f" - Added model '{args.top}' in VPR arch file at {arch_dir}/openfpga_arch.xml")
    ...
    tree.write(f'{arch_dir}/openfpga_arch.xml', pretty_print=True, xml_declaration=True, encoding='UTF-8')
    print(f" - Added tile '{args.top}_tile' in VPR arch file at {arch_dir}/openfpga_arch.xml")
    ...
    # tree.write(f'{arch_dir}/openfpga_arch.xml')
    tree.write(f'{arch_dir}/openfpga_arch.xml', pretty_print=True, xml_declaration=True, encoding='UTF-8')

    print(f" - Added pb_type '{args.top}_tile' in VPR arch file at {arch_dir}/openfpga_arch.xml")

    print(f"Completed writing VPR arch file with '{args.top}' as a hard block at {arch_dir}/openfpga_arch.xml")


###########################################################


def gen_openfpga_arch_file(args):
    if args.output_dir:
    ...
    shutil.copy(f'{args.openfpga_arch}', f'{task_dir}/openfpga_arch.xml')

    update_xml(port_dict, args.top, has_clock, clock_port, task_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--openfpga_arch', required=True, help='Path to the base VPR architecture template file')
    args = parser.parse_args()

    gen_openfpga_arch_file(args)