...
def gen_vpr_arch_file(args):
    ...
    shutil.copy(f'{args.vpr_arch}', f'{task_dir}/vpr_arch.xml')

    # Parse the XML file
    tree = ET.parse(f'{task_dir}/vpr_arch.xml')
    root = tree.getroot()
    ...
    tree.write(f'{task_dir}/vpr_arch.xml')
    print(f" - Added model '{args.top}' in VPR arch file at {task_dir}/vpr_arch.xml")
    ...
    tree.write(f'{task_dir}/vpr_arch.xml')
    print(f" - Added tile '{args.top}_tile' in VPR arch file at {task_dir}/vpr_arch.xml")
    ...
    tree.write(f'{task_dir}/vpr_arch.xml')
    print(f" - Added pb_type '{args.top}_tile' in VPR arch file at {task_dir}/vpr_arch.xml")

    print(f"Completed writing VPR arch file with '{args.top}' as a hard block at {task_dir}/vpr_arch.xml")


if __name__ == "__main__":
    args = parse_arguments()
    gen_vpr_arch_file(args)