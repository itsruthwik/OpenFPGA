...
<architecture>
  ...
  <layout tileable="true">
    ...
    <fixed_layout name="3x4" width="6" height="8">
      <!--Perimeter of 'io' blocks with 'EMPTY' blocks at corners-->
      <perimeter type="io" priority="100"/>
      <corners type="EMPTY" priority="101"/>
      <!--Fill with 'clb'-->
    ...
    <pb_type name="router">
      ...
      <mode name="router_slice">
        ...
        <interconnect>
          ...
          <complete name="clk" input="router.clk" output="router_slice.clk"/>

          <complete name="reset" input="router.reset" output="router_slice.reset"/>

          <direct name="router_address2router_address" input="router.router_address" output="router_slice.ROUTER_ADDRESS">
            <delay_constant max="134e-12" min="74e-12" in_port="router.router_address" out_port="router_slice.ROUTER_ADDRESS"/>
          </direct>
          <direct name="channel_in_ip2channel_in_ip" input="router.channel_in_ip" output="router_slice.CHANNEL_IN_IP">
            <delay_constant max="134e-12" min="74e-12" in_port="router.channel_in_ip" out_port="router_slice.CHANNEL_IN_IP"/>
          </direct>
          <direct name="flow_ctrl_in_op2flow_ctrl_in_op" input="router.flow_ctrl_in_op" output="router_slice.FLOW_CTRL_IN_OP">
            <delay_constant max="134e-12" min="74e-12" in_port="router.flow_ctrl_in_op" out_port="router_slice.FLOW_CTRL_IN_OP"/>
          </direct>
          <direct name="error2error" input="router_slice.ERROR" output="router.error">
            <delay_constant max="1.93e-9" min="74e-12" in_port="router_slice.ERROR" out_port="router.error"/>
          </direct>
          <direct name="channel_out_op2channel_out_op" input="router_slice.CHANNEL_OUT_OP" output="router.channel_out_op">
            <delay_constant max="1.93e-9" min="74e-12" in_port="router_slice.CHANNEL_OUT_OP" out_port="router.channel_out_op"/>
          </direct>
          <direct name="flow_ctrl_out_ip2flow_ctrl_out_ip" input="router_slice.FLOW_CTRL_OUT_IP" output="router.flow_ctrl_out_ip">
            <delay_constant max="1.93e-9" min="74e-12" in_port="router_slice.FLOW_CTRL_OUT_IP" out_port="router.flow_ctrl_out_ip"/>
          </direct>
        </interconnect>
      </mode>
